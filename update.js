const fs = require('fs');
const path = 'g:\\\\Мій диск\\\\Claude\\\\SolarKP HTML\\\\solarkp\\\\index.html';
...
// Make result panel scrollable
code = code.replace('.result-panel{position:sticky;top:80px;height:fit-content;display:flex;flex-direction:column;gap:16px}',
                    '.result-panel{position:sticky;top:80px;height:fit-content;max-height:calc(100vh - 100px);overflow-y:auto;padding-right:5px;display:flex;flex-direction:column;gap:16px}');

// Add loader in HTML
code = code.replace('<body>', '<body>\\n<div id="loader" class="loader-overlay"><div class="spinner"></div><div class="loader-text">Отримання актуальних цін з бази...</div></div>');

// Replace constants with dynamic parser blocks
const constantPattern = /const PANELS=\\[.*?\\];\\s*const H_INV=\\{.*?\\};\\s*const G_INV=\\{.*?\\};\\s*const MOUNT=\\{.*?\\};/s;
const dynamicVars = `let PANELS = [];
let H_INV = {};
let G_INV = {};
let MOUNT = {
...
code = code.replace(constantPattern, dynamicVars);
...
// Modify calcCapex hardwareNoVAT logic and CABLE_P
...
code = code.replace('const opts=PANELS.map((pl,idx)=>\`<option value="\${idx}"\${S.panelIdx==idx?" selected":""}>\${pl.label} · $\${(pl.price*pl.power).toFixed(2)}/шт [\${pl.avail}]</option>\`).join("");',
                    'const opts=PANELS.map(pl=>`<option value="\${esc(pl.model)}"\${S.panelModel===pl.model?" selected":""}>\${pl.label} · $\${(pl.price*pl.power).toFixed(2)}/шт [\${pl.avail}]</option>`).join("");');
      ...
      <div class="fld" onclick="toggleHardwareVAT()">
        <div class="chk-row\${S.hardwareNoVAT?" chk":""}">
          ...
          <div>
            ...
            <div class="chk-sub">ПДВ 20% не буде нараховуватись на вартість панелей та інверторів</div>
          </div>
        </div>
      </div>`;
...
// Update VAT Label in KP
code = code.replace('["ПДВ 20%",C.sesVAT,"tr-dim"],', '[(S.hardwareNoVAT?"ПДВ 20% (окрім панелей та інверторів)":"ПДВ 20%"),C.sesVAT,"tr-dim"],');

// Show Note in KP if without VAT
...
// Update 'Без ПДВ' text in Table dynamically
...
code = code.replace('<td>${r.name}</td>', '<td>${r.name}</td>');

// Wait! r.name was changed exactly above, but it's used directly in HTML.
// We change: `name:\`...`, then we append, then we render. It works.

// Append Fetch Logic before BuildUI
const fetchInit = `
...
async function initApp() {
  try {
    ...
    p.forEach(r=>{
      if(r.length>=5){
        const model=r[0]?.trim(), desc=r[1]?.trim(), avail=r[2]?.trim(), priceStr=r[4];
    ...
    const parseI=(csv, map)=>{ csv.forEach(r=>{
      if(r.length>=5){
        const model=r[0]?.trim(), desc=r[1]?.trim(), priceStr=r[4];
    ...
    c.forEach(r=>{ const desc=r[1]?.trim(), priceStr=r[4]; if(desc&&desc.includes("1*6")&&priceStr){ const p=parsePrice(priceStr); if(p>0) CABLE_P=p*1000; }});

    m.forEach(r=>{
      const desc=r[1]?.trim(), priceStr=r[4];
      if(desc&&priceStr){
